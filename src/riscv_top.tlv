...
\TLV
   ...
   $imm[31:0] = $is_i_instr ? {{21{$instr[31]}}, $instr[30:25], $instr[24:21], $instr[20]}:
                ...
                $is_u_instr ? {$instr[31], $instr[30:30], $instr[19:12], 12'b0}:
   ...
   $is_load  = $is_lb | $is_lh | $is_lw | $is_lhu;
   ...
   // Data Memory
   $addr_dmem[4:0] = $src1_value + $imm[4:0];
   $wr_en_dmem = $is_s_instr;
   $rd_en_dmem = $is_load;
   $wr_data_dmem[31:0] = $src2_value;
   m4+dmem(32, 32, $reset, $addr_dmem[4:0], $wr_en_dmem, $wr_data_dmem[31:0], $rd_en_dmem, $ld_data[31:0]) 
   
   // Register field instantiation and visualization (debug)
   m4+rf(32, 32, $reset, $rd_valid, $rd, $result, $rs1_valid, $rs1, $src1_value[31:0], $rs2_valid, $rs2, $src2_value[31:0])
   
   `BOGUS_USE ($rd $rd_valid $rs1 $rs1_valid $rs2 $rs2_valid $opcode $funct3 $dec_bits $is_beq)
